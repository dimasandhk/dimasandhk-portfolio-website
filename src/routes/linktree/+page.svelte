<script lang="ts">
	...
	onMount(async () => {
		...
		// ⚡ BOLT OPTIMIZATION: Extract unrelated concurrent asynchronous operations into separate execution paths.
		// Awaiting them independently allows each UI component to render as soon as its data is available,
		// improving perceived performance and Time to Interactive (TTI).

		// Fetch Now Playing independently
		fetch('/api/spotify/now-playing')
			...
			.catch((e) => console.error('Failed to fetch now playing data', e))
			.finally(() => {
				isNowPlayingLoading = false;
			});

		// Fetch Top Tracks independently
		fetch('/api/spotify/top-tracks')
			...
			.catch((e) => console.error('Failed to fetch top tracks data', e))
			.finally(() => {
	...
	<NotionBlock>
		...
		<div class="mb-6">
			...
			{#if isNowPlayingLoading}
				<div class="text-[#9b9a97] animate-pulse text-sm">Loading current track...</div>
