<script lang="ts">
	...
	onMount(async () => {
		...
		fetch('/api/spotify/top-tracks')
			...
			.finally(() => (isTopTracksLoading = false));
	});

	// ⚡ Bolt: Cache Intl.DateTimeFormat instance to avoid recreating it in loop
	const videoDateFormatter = new Intl.DateTimeFormat();
</script>
					...
					>
						...
						<div class="p-3">
							...
							<div class="text-xs text-[#9b9a97] mt-1">
								{videoDateFormatter.format(new Date(video.snippet.publishedAt))}
							</div>
						</div>
					</div>
				</NotionBlock>
	...
	<NotionBlock>
		...
		<div class="mb-6">
			...
			{#if isNowPlayingLoading}
				<div class="text-[#9b9a97] animate-pulse text-sm">Loading currently playing...</div>
