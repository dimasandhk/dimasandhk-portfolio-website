<script lang="ts">
	...
	// ⚡ Bolt: Precompute formatted dates with a hoisted Intl.DateTimeFormat instance to avoid recreating it in loop
	const dateFormatter = new Intl.DateTimeFormat();
	let formattedVideos = $derived(
		videos.map((video) => ({
			...video,
			formattedPublishedAt: dateFormatter.format(new Date(video.snippet.publishedAt))
		}))
	);
