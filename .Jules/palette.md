...
## 2025-02-28 - Keyboard Parity for Hover Effects

**Learning:** When using Tailwind's `group-hover` utility to create visual effects (like changing text color, scaling images, showing icons, or displaying tooltips), those effects are only visible to mouse users. Keyboard users miss out on these visual cues when they tab to the element, which can hide important context (like tooltips or external link indicators).
**Action:** Always pair `group-hover` modifiers with `group-focus-visible` modifiers on child elements to ensure keyboard parity. For example, use `group-hover:opacity-100 group-focus-visible:opacity-100` so that tooltips and icons appear when the parent element receives keyboard focus.
