...
## 2025-03-01 - Interactive Group Hover and Keyboard Parity

**Learning:** Found several interactive elements (like the "View More" placeholder links in the gallery, Spotify link components, and tooltips) that used `group-hover:*` Tailwind utilities for visual flair (opacity changes, scaling, background colors) but did not have equivalent behavior when focused via keyboard. A user tabbing through elements wouldn't see the same dynamic UI changes as a mouse user.
**Action:** When applying `group-hover:*` to interactive components, always pair it with equivalent `group-focus-visible:*` classes (e.g., `group-hover:scale-105 group-focus-visible:scale-105`) to ensure consistent visual feedback across different interaction methods.
