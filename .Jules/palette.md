...
## 2024-05-15 - Keyboard Parity for Hover States

**Learning:** Hover states implemented using `group-hover` in Tailwind often lack keyboard navigation equivalents. Screen reader or keyboard-only users will miss tooltips or visual interactions when tabbing through if only `group-hover` is applied.
**Action:** Always pair `group-hover` styles with `group-focus-visible` variants for tooltips, icons, and visual effects to ensure full keyboard navigation parity across interactive elements.
