...
\n## 2026-04-20 - Focus-Visible Utility Standard\n\n**Learning:** Found several interactive elements (links, buttons) missing the standard project focus indicators, particularly within modals and secondary navigation paths. A consistent visual focus ring is crucial for keyboard navigation.\n**Action:** Applied the standard Tailwind focus ring (`focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--notion-bg)]`) to all previously unstyled interactive elements. When an element is placed on top of an image (like a modal close button), use `focus-visible:ring-white` instead of gray for better contrast.

## 2025-02-28 - Mouse and Keyboard Interaction Parity on Groups

**Learning:** Found several complex interactive elements (like custom links with icons, styled placeholders, and gallery cards) that heavily used Tailwind's `group-hover` variant for visual state changes (showing icons, scaling images, changing text color) but lacked corresponding keyboard focus feedback. Using only `group-hover` means keyboard users tab to the element but don't see the same state change, leading to an inconsistent and less accessible experience.
**Action:** When building custom interactive components using the `group` class, always pair `group-hover:*` utility classes with `group-focus-visible:*` classes to ensure that visual feedback is identical for both mouse users hovering and keyboard users focusing. Additionally, always explicitly apply standard `focus-visible:ring-2` styles to the parent interactive element.
