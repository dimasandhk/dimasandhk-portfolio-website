...
## 2025-05-17 - Complete Mouse/Keyboard Parity for Interactive States

**Learning:** When using Tailwind's `group-hover` to trigger visual changes in child elements (like tooltips appearing, images scaling, or icons changing color), keyboard users miss these entirely if only `focus-visible` is applied to the parent. This creates an inconsistent and inferior experience for keyboard navigation.
**Action:** Always pair `group-hover:{utility}` with `group-focus-visible:{utility}` to ensure exact 1:1 parity between mouse hover and keyboard focus states, making the UI delightful and fully accessible for all users.
