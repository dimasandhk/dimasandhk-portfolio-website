...
## 2026-06-29 - Keyboard Interaction Parity for Grouped States

**Learning:** Found instances where interactive elements used Tailwind `group-hover` utility classes to reveal content (like tooltips or external link icons) or scale images, but failed to include corresponding `group-focus-visible` utility classes. This meant that while mouse users saw these enhancements on hover, keyboard users tabbing into the elements saw nothing. Furthermore, custom modal backdrops failed to implement the Escape key closing behavior, trapping keyboard users.
**Action:** Always pair `group-hover` state variants with `group-focus-visible` variants to ensure interaction parity between mouse and keyboard users. Additionally, any custom modal backdrops must handle `Escape`, `Enter`, and `Space` key events explicitly in their `onkeydown` handlers to be accessible.
