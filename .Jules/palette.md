...
## 2026-04-27 - Keyboard Hover Parity for Interactive States

**Learning:** Found multiple instances where visual feedback for interactivity (e.g., showing a tooltip, revealing an external link icon, scaling an image) was only tied to mouse interactions using `group-hover`. This left keyboard users without the same context or visual delight when focusing on elements.
**Action:** Always pair `group-hover` variants with `group-focus-visible` variants (e.g., `group-hover:opacity-100 group-focus-visible:opacity-100`) to ensure parity between mouse and keyboard navigation experiences.
