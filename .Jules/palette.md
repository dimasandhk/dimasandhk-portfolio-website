...
## 2026-05-11 - Keyboard Focus Parity for Visual Hover States

**Learning:** Found multiple instances where visual feedback elements (e.g., tooltips, external link icons, image scaling) were triggered exclusively via `group-hover:*` utility classes. This meant keyboard-only users navigating via Tab missed critical visual cues indicating interactivity or extra context.
**Action:** Always pair `group-hover:*` visual state classes with their `group-focus-visible:*` equivalents to ensure visual accessibility and parity between mouse and keyboard navigation.
